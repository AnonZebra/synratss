...
consistency_scoring_swe <- function(part_df, plotdir=NULL, method="euclidean",
                                fmt="Luv", nameby=NULL) {
  ...
  for (foo in 1:nrow(part_df)){
    ...
    if (!is.null(plotdir)) {
      if (is.null(nameby)) {
        plot_syn_cons_swe(out, hexcolors, # calls the plot_syn_cons function to produce a plot for the participant in the specified "plotdir" directory.
