...
#'
#' @description Converts Swedish personal numbers (in the format "YYYYMMDD-XXXX",
...
pnr_to_bdate <- function(pnr){
  bdate <- pnr
  if (is.na(bdate)) {return(NA)}
  bdate <- sub("-.*$", "", pnr)
  if (nchar(bdate) != 6 & nchar(bdate) !=8) {return("Invalid pnr format")}
  if (nchar(bdate) == 6) {
